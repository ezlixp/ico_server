...
tomeRouter.get("/", async (request: Request, response: Response) => {
    try {
        ...
        const tomeList = await guildDatabases[request.wynnGuildId!].TomeModel.find({}).sort({ dateAdded: 1 });
...
tomeRouter.get("/:username", async (request: Request<{ username: string }>, response: Response) => {
    try {
        ...
        const result = await guildDatabases[request.wynnGuildId!].TomeModel.findOne({
        ...
        const position =
            (await guildDatabases[request.wynnGuildId!].TomeModel.find({
...
tomeRouter.post(
    ...
    async (request: Request<{}, {}, { username: string }>, response: Response) => {
        try {
            ...
            const exists = await guildDatabases[request.wynnGuildId!].TomeModel.findOne({
            ...
            const tome = new guildDatabases[request.wynnGuildId!].TomeModel(tomeData);
            await tome.save();
...
tomeRouter.delete(
    ...
    async (request: Request<{ username: string }>, response: Response) => {
        try {
            ...
            const result = await guildDatabases[request.wynnGuildId!].TomeModel.findOneAndDelete({
                username: username,
            }).collation({
                locale: "en",
                strength: 2,
